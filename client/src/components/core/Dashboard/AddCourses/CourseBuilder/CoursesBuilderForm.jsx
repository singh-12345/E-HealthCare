...
import { setCourses , setStep , setEditCourse } from '../../../../../slices/courseSlice';
import NestedView from './NestedView';
import { MdNavigateNext } from 'react-icons/md';
import {toast} from "react-hot-toast"
import { createSection , updateSection } from '../../../../../services/operations/courseDeatils';
...
const CoursesBuilderForm = () => {
    ...
    const goToNext = () =>{
      ...
      if(course.courseContent.some((section)=>section.subSection.length === 0)){
        toast.error("Please add atlest one video in each section")
        return 
      }
      dispatch(setStep(3));
    }
  ...
  return (
    <div className=' space-y-8 rounded-md border-[1px] border-richblack-700 bg-richblack-800 p-6'>
        ...
        <form className=' space-y-4' onSubmit={handleSubmit(onSubmit)}>
          ...
          id='sectionName'
          disabled={loading}
          placeholder='Add a section to build your courses'
          ...
          <div className=' flex items-center gap-x-4'>
            ...
            type="submit"
            disabled={loading}
            text={editSectionName !==null ? "Edit Section name " : "Create Section"}
            outline={true}
            > <IoAddCircleOutline size={20} className="text-yellow-50"></IoAddCircleOutline></IconBtn>
            {editSectionName && (
              <button 
              type='button'
              onClick={cancelEdit}
