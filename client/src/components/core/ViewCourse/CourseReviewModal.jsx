...
import { createRating } from '../../../services/operations/courseDeatils'

const CourseReviewModal = ({setReviewModal}) => {
  const {user} = useSelector((state)=>state.profile);
  const {token} = useSelector((state)=>state.auth);
  const {courseEntireDate} = useSelector((state)=>state.viewCourse);
  ...
  return (
    ...
    overflow-auto bg-white bg-opacity-10 backdrop-blur-sm'>
      ...
      {/* Modal header */}
      <div className='flex p-5 items-center justify-between rounded-t-lg bg-richblack-700'>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className='mt-6 flex flex-col items-center'>
        ...
        <div className='mt-6 flex w-11/12 justify-end gap-x-2 mb-4 '>
        <button onClick={()=>setReviewModal(false)}
